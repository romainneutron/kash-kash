...
import 'package:kash_kash_app/domain/usecases/abandon_quest_use_case.dart';
...
void main() {
  ...
  late AbandonQuestUseCase useCase;
  ...
  setUp(() {
    ...
    useCase = AbandonQuestUseCase(mockRepository);
  });

  group('AbandonQuestUseCase', () {
    test('should abandon attempt when in progress', () async {
      ...
      when(() => mockRepository.abandonAttempt(attemptId))
      ...
      result.fold(
        ...
        (attempt) {
          ...
          expect(attempt.status, AttemptStatus.abandoned);
        },
      );

      verify(() => mockRepository.getAttemptById(attemptId)).called(1);
      verify(() => mockRepository.abandonAttempt(attemptId)).called(1);
    });
    ...
    test('should return failure when attempt is already completed', () async {
      ...
      verifyNever(() => mockRepository.abandonAttempt(any()));
    });

    test('should return failure when attempt is already abandoned', () async {
      const attemptId = 'attempt-1';
      final abandonedAttempt = createTestAttempt(
        id: attemptId,
        status: AttemptStatus.abandoned,
    ...
    test('should return failure when abandonAttempt fails', () async {
      const attemptId = 'attempt-1';
      final inProgressAttempt = createTestAttempt(
        id: attemptId,
        status: AttemptStatus.inProgress,
